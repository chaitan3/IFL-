    ...
    if(($sum<=1000) && $flag)
    {
            ...
            if($flag==0)
            {
                if(($def==5)&&($mid==5)&&($gk==2)&&($for==3))
                {
                    ...
                    if($flag==0)
                    {
                        $query='insert into fpl values("'.$_SESSION['user'].'",';
                        for($i=0;$i<15;$i++)
                            $query=$query.$row[$i]['id'].',';
                        $query=$query.'0,'.$row[0]['id'].','.(1000-$sum).',0)';
...
<div id="fpl_content">
<a style="float:right" class="button" href="logout.php"><span>Logout</span></a>
<h4>Select 15 players for your FPL team    </h4><br>
<ul class="left" style="margin-left:20px">
<li>You'll have a total of 1000 points to select your players.</li>
<li>From the 15 players that you select there have to be 5 defenders 5 midfielders 3 strikers and 2 goalkeepers</li>
<li>You can have at max 3 players from one team</li>
<li>Click on Submit button after selecting 15 players.</li>
</ul>
<br>
<h4>Current Points: <p id="points">1000</p><br></h4>
<div id="list">
<form action="maketeam.php" method="post" id="form1" onsubmit="return check15_submit()">
